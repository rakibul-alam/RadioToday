        ...
        <table class="datatable-init nowrap table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <img src="{{ asset('storage/segments/'.$segment->image) }}" alt="" width="50px">
                    </td>
                    <td class="text-wrap">
                        <span id="content1">{!! substr(strip_tags($segment->details), 0, 150) !!}</span>
                    </td>
                    <td>
                        <div class="custom-control custom-switch status-change"
                        data-value="{{ $segment->status == StatusEnum:: Active->value ? StatusEnum:: Inactive->value : StatusEnum:: Active->value }}"
                        data-url="{{route('segments.status.change',$segment->id)}}">
                        <input type="checkbox" class="custom-control-input" id="customSwitch{{$index + 1}}"
                            {{ $segment->status == StatusEnum::Active->value ? 'checked' : '' }} name='input_switch'>
                    ...
                    <td>
                        <form method="POST" action="{{ route('segments.destroy', $segment->id) }}">
                            ...
                            <a href="javascript:void(0)" class="badge bg-outline-info ajax-modal-trigger"
                                data-route="{{ route('segments.edit', $segment->id) }}"><em
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Segment Info</h5>
            ...
            <div class="modal-body col-md-12">
                <form action="{{route('segments.store')}}" method="post" class="form-validate is-alter"
