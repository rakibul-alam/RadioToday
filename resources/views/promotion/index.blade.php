        ...
        <table class="datatable-init nowrap table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <img src="{{ asset('storage/promotions/'.$promotion->image) }}" alt="" width="50px">
                    </td>
                    <td class="text-wrap">
                        <span id="content1">{!! substr(strip_tags($promotion->details), 0, 150) !!}</span>
                    </td>
                    <td>
                    <div class="custom-control custom-switch status-change"
                        data-value="{{ $promotion->status == StatusEnum:: Active->value ? StatusEnum:: Inactive->value : StatusEnum:: Active->value }}"
                        data-url="{{route('promotions.status.change',$promotion->id)}}">
                        <input type="checkbox" class="custom-control-input" id="customSwitch{{$index + 1}}"
                            {{ $promotion->status == StatusEnum::Active->value ? 'checked' : '' }} name='input_switch'>
                    ...
                    <td>
                        <form method="POST" action="{{ route('promotions.destroy', $promotion->id) }}">
                            ...
                            <a href="javascript:void(0)" class="badge bg-outline-info ajax-modal-trigger"
                                data-route="{{ route('promotions.edit', $promotion->id) }}"><em
                                    class=" icon ni ni-edit"></em>
                                Edit</a>

                            <!-- <a href="{{ route('promotions.show',$promotion->id) }}" class="badge bg-outline-success"><em
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Promotion Info</h5>
            ...
            <div class="modal-body col-md-12">
                <form action="{{route('promotions.store')}}" method="post" class="form-validate is-alter"
